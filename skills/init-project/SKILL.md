...
**Create `project.yml`** in the project root (replace placeholders with answers from Step 1):

```yaml
name: PROJECT_NAME
options:
  bundleIdPrefix: BUNDLE_ID_PREFIX   # e.g. com.vlad
  deploymentTarget:
    iOS: "17.0"
  xcodeVersion: "15"
...
settings:
  base:
    SWIFT_VERSION: "5.9"
    DEVELOPMENT_TEAM: ""             # fill in after Xcode login
...
targets:
  PROJECT_NAME:
    ...
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: BUNDLE_ID_PREFIX.PROJECT_NAME_LOWER
  ...
  PROJECT_NAMETests:
    ...
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: BUNDLE_ID_PREFIX.PROJECT_NAME_LOWERTests
```
...
**Create `app/PROJECT_NAMEApp.swift`** (minimal entry):
```swift
import SwiftUI
...
**Add `.claude/settings.local.json`** (allow build tools):
```json
{
  "permissions": {
    "allow": [
